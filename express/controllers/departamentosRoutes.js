...
router.post("/departamentos", (req, res) => {
  ...
  const query = "INSERT INTO departamentos (id, nome) VALUES ($1, $2)";
...
router.post("/departamentos/update", (req, res) => {
  const { id, nome } = req.body;
  const query = "UPDATE departamentos SET nome = $1 WHERE id = $2";
...
router.post("/departamentos/delete", (req, res) => {
  ...
  const query = "DELETE FROM departamentos WHERE id = $1";
